...
import { insertE2EGetAllDeliveriesReceptionsReleasedTestData } from "../../../test_data/e2e/deliveries_receptions_test_data";

describe("GET /api/deliveries-receptions/released", () => {
    let app: Express;
    ...
    beforeAll(async () => {
        ...
        const { zoneManagerAndWitnessUser1, deliveryReception } =
            await insertE2EGetAllDeliveriesReceptionsReleasedTestData();
    ...
    it("Should return all released deliveries receptions for a zone manager", async () => {
        const response = await request(app)
            .get("/api/deliveries-receptions/released")
    ...
    it("Should return an error for missing token", async () => {
        const response = await request(app).get(
            "/api/deliveries-receptions/released"
        );
